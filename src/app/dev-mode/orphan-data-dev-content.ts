...
export const orphanDataHandlingDocument: DevDocument = {
  id: 'orphan-data-handling-strategy',
  title: 'Orphan Data Handling Strategy',
  sourceUrl: '/docs/product/orphan-data-handling-strategy.md',
  markdown: `# Orphan Data Handling Strategy for Referenced Fields
...
The UI should explain that the value is no longer available, that an active value can replace it, and that replacement is irreversible through the normal UI.

## Selection Behavior

Only active referenced records may appear in dropdowns, search pickers, trees, or typeaheads for new assignments.

Unavailable or orphaned references remain visible when already stored, but are not selectable for new assignments or replacements.
