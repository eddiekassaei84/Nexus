...
// ─── Props ────────────────────────────────────────────────────────────────────
export interface ExportRefDataModalProps {
  isOpen: boolean;
  onClose: () => void;
  tabLabel: string;
...
// ─── Main Component ───────────────────────────────────────────────────────────
export function ExportRefDataModal({
  isOpen,
  onClose,
  tabLabel,
  structureType,
...
}: ExportRefDataModalProps) {
  const makeDefault = () =>
    `${projectName.replace(/\s+/g, '')}-${tabLabel.replace(/\s+/g, '')}-${makeTimestamp()}`;
  ...
  function handleExport() {
    ...
    if (structureType === 'hierarchy') {
      headers = ['Code', 'Title', 'Level', 'Note'];
      rows = flattenItems(items);
    } else {
      headers = ['Code', 'Name', 'Type', 'Note'];
      rows = flatGroups.flatMap(g => [
        [g.code, g.name, groupLabel, g.description ?? ''],
        ...g.children.map(c => [c.code, c.name, itemLabel, c.description ?? '']),
      ]);
    }
    ...
    a.href     = url;
    a.download = `${filename || `${tabLabel}-export`}.csv`;
    a.click();
  ...
  const totalRows = structureType === 'hierarchy'
    ? flattenItems(items).length
    : flatGroups.reduce((sum, g) => sum + 1 + g.children.length, 0);

  const columnTags = structureType === 'hierarchy'
    ? ['Code', 'Title', 'Level', 'Note']
    : ['Code', 'Name', 'Type', 'Note'];
      ...
      >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 36, padding: '9px 24px 24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <p style={{ fontFamily: 'Inter, sans-serif', fontWeight: 400, fontSize: 14, lineHeight: '20px', color: '#595959', margin: 0 }}>
                Export your <strong style={{ fontFamily: 'Open Sans, sans-serif', fontWeight: 600, color: '#384857' }}>{tabLabel}</strong> reference data to CSV for review, editing, or sharing.
                {structureType === 'hierarchy'
                  ? ' The file will include all items with their hierarchy levels.'
                  : ` The file will include all ${groupLabel.toLowerCase()}s and ${itemLabel.toLowerCase()}s.`}
                {totalRows > 0 && (
                  <>{' '}<strong style={{ fontFamily: 'Open Sans, sans-serif', fontWeight: 600, color: '#384857' }}>{totalRows}</strong> row{totalRows !== 1 ? 's' : ''} will be exported.</>
                )}
              </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {columnTags.map(col => (
                    <span key={col} style={{ fontFamily: 'Inter, sans-serif', fontSize: 12, fontWeight: 400, color: '#384857', backgroundColor: '#F2F3F4', border: '1px solid #D9D9D9', borderRadius: 4, padding: '2px 8px' }}>
                      {col}
                    </span>
                  ))}
                </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'stretch', width: '100%' }}>
                  <div style={{ flex: 1, backgroundColor: '#FFFFFF', display: 'flex', alignItems: 'center', padding: '0 12px', minHeight: 40, borderRadius: '4px 0 0 4px', border: `1px solid ${inputFocused ? '#91D5FF' : '#9EA2A8'}`, borderRight: 'none', transition: 'border-color 0.15s' }}>
                    <input
                  ...
                  <div style={{ backgroundColor: '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 12px', minHeight: 40, borderRadius: '0 4px 4px 0', flexShrink: 0, width: 60, border: '1px solid #9EA2A8', borderLeft: 'none' }}>
                    <span style={{ fontFamily: "'Actor', sans-serif", fontWeight: 400, fontSize: 14, lineHeight: '1.2', color: '#9EA3A9', userSelect: 'none' }}>.csv</span>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div style={{ backgroundColor: '#F5F5F5', height: 72, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8, paddingRight: 24, borderTop: '1.111px solid #C3C7CC' }}>
          <button onClick={handleExport}
            style={{ height: 36, paddingLeft: 16, paddingRight: 16, backgroundColor: '#FF4D00', border: 'none', borderRadius: 4, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Inter, sans-serif', fontSize: 14, fontWeight: 400, lineHeight: '20px', color: '#FFFFFF', whiteSpace: 'nowrap', transition: 'background-color 0.15s', flexShrink: 0 }}
